"""
Convert Q-learning pickle file to JSON format.
...
def convert_q_table_to_json(pkl_file='q_learning_tictactoe.pkl', output_file='q_table.json'):
    """
    Convert Q-learning pickle file to JSON format.
    ...
    """
    print(f"Loading Q-table from {pkl_file}...")
    ...
    for state_key, actions_dict in agent.q_table.items():
        # Handle new Q-table structure: key is (state, player) tuple
        ...
        if isinstance(state_key, tuple) and len(state_key) == 2:
            ...
            if isinstance(first_elem, tuple) and isinstance(second_elem, int):
                # New format: (state_tuple, player)
                state_tuple, player = state_key
            else:
                # Old format: just state_tuple (backward compatibility)
        ...
        if valid_indices:
            ...
            # No valid moves or Q-values, set uniform probabilities
            if valid_moves:
...
if __name__ == "__main__":
    ...
    pkl_file = sys.argv[1] if len(sys.argv) > 1 else 'q_learning_tictactoe.pkl'
    output_file = sys.argv[2] if len(sys.argv) > 2 else 'q_table.json'
    
    convert_q_table_to_json(pkl_file, output_file)
